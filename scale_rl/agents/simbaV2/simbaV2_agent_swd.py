"""
SimbaV2 Agent with Scheduled Weight Decay (AdamS) optimizer.

This variant of the SimbaV2 agent uses the AdamS optimizer from
"Stable Weight Decay Regularization" instead of standard Adam.
"""
...
from scale_rl.agents.jax_utils.adams_optax import adams
...
class SimbaV2SWDConfig:
    """Configuration for SimbaV2 with Scheduled Weight Decay."""
    seed: int
...
def _init_simbav2_swd_networks(
    observation_dim: int,
    action_dim: int,
    cfg: SimbaV2SWDConfig,
) -> Tuple[PRNGKey, Network, Network, Network, Network]:
    """Initialize networks with AdamS optimizer."""
    ...
    # Actor network with AdamS optimizer
    ...
    # Critic network with AdamS optimizer
    ...
    # Temperature network with AdamS optimizer
...
@jax.jit
def _sample_simbav2_swd_actions(
    rng: PRNGKey,
    actor: Network,
...
def _update_simbav2_swd_networks(
    rng: PRNGKey,
    actor: Network,
    critic: Network,
...
class SimbaV2AgentSWD(BaseAgent):
    """
    SimbaV2 Agent with Scheduled Weight Decay (AdamS) optimizer.

    This agent is identical to SimbaV2Agent but uses the AdamS optimizer
    which implements scheduled weight decay based on global gradient magnitude.
    """
    ...
    def __init__(
        ...
        super(SimbaV2AgentSWD, self).__init__(
            observation_space,
            action_space,
            cfg,
        )
        ...
        self._cfg = SimbaV2SWDConfig(**cfg)
    ...
    def sample_actions(
        ...
        self._rng, actions = _sample_simbav2_swd_actions(
